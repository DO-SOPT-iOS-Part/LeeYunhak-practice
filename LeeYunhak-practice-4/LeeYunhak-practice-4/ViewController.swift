...
class ViewController: UIViewController {
    ...
    @objc private func registerButtonTap() {
        Task {
            if let result = try await RegisterService.shared.PostRegisterData(userName: idTextField.text ?? "test") {
                isExist = result.isExist
            }
            if isExist {
                print("isExist!!")
                let alert = UIAlertController(title: "중복입니다.", message: "흑흑", preferredStyle: UIAlertController.Style.alert)
                let okAction =  UIAlertAction(title: "확인", style: UIAlertAction.Style.default)
                alert.addAction(okAction)
                self.present(alert, animated: true)
            } else {
                do {
                    let status = try await RegisterService.shared.PostRegisterData(userName: self.usreName,
